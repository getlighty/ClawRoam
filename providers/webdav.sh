...
set -euo pipefail
VAULT_DIR="$HOME/.clawvault"; PROVIDER_CONFIG="$VAULT_DIR/.provider-webdav.json"
...
cmd_push() {
  ...
  log "Pushing to WebDAV..."
  rclone sync "$VAULT_DIR" "${RCLONE_REMOTE}:${rdir}" $EXCLUDE -v 2>&1 | grep -E "Transferred|Elapsed" || true
  log "Push to WebDAV complete"
}
...
cmd_pull() {
  ...
  local rdir
  rdir=$(python3 -c "import json;print(json.load(open('$PROVIDER_CONFIG'))['remote_dir'])" 2>/dev/null || echo "$REMOTE_DIR")
  log "Pulling from WebDAV..."
  rclone sync "${RCLONE_REMOTE}:${rdir}" "$d" -v 2>&1 | grep -E "Transferred|Elapsed" || true
  ...
  rm -rf "$d"; log "Pull from WebDAV complete"
}
